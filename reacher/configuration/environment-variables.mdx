...
<ParamField path="PROXY_ALLOWED_DOMAINS" type="string" required>
  ...
  Matching is done against the exact hostname parsed from the request URL. See [Domain allowlist](/configuration/domain-allowlist) for a full explanation.
</ParamField>
...
<ParamField path="FETCH_EXTERNAL_TOKEN_MAP" type="string" default='{}' >
  ...
  The token value never leaves the server — Claude only sees the result of the API call. See [Domain allowlist](/configuration/domain-allowlist) for examples.
</ParamField>
...
See [reacher.config.yaml reference](/configuration/reacher-config) for full details on the YAML config file.
